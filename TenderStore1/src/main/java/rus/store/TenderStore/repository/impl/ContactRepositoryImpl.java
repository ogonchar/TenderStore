	...
	public void addContactByParcer(Tender tender, String owner) {
		...
		try {
			...
			Contact contact = new Contact(); 
				...
				if ((Contact) session.get(Contact.class, contact.getIdInn()) == null) {
				...
				log.warn("Trying to add owner");
				contact.setOwner(owner);
				log.warn(contact.getOwner());
					if (contact.getIdInn()!=0) {
						session.save(contact);
					session.getTransaction().commit();
					}else log.warn("Unable to add contact,  id is Empty");
				}else log.warn("Contact already exist");
				
		} catch (IOException e1) {
			log.warn("Problem in parcing in common");
		} catch (ConstraintViolationException e){
			log.warn("Duplicate entry, tender has not beeing added");
		} finally{
			session.close();			
		}
	}
